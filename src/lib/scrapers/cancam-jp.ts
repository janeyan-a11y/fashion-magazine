...
export class CanCamScraper extends BaseScraper {
  name = "CanCam";
  scrapeUrl = "https://cancam.jp/magazine/";
  ...
  parseArticles(html: string): ScrapedArticle[] {
    ...
    $("article, .magazine-content a, .post-item").each((_, el) => {
      const $el = $(el);
      ...
      if (title && title.length > 2) {
        articles.push({
          title,
          imageUrls: img ? [img] : [],
          sourceUrl: link.startsWith("http") ? link : `https://cancam.jp${link}`,
        });
      }
    });
