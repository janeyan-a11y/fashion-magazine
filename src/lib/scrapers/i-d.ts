...
export class IDMagazineScraper extends BaseScraper {
  name = "i-D Magazine";
  scrapeUrl = "https://i-d.co/fashion/";
  ...
  parseArticles(html: string): ScrapedArticle[] {
    ...
    $("article, .post-item, .story-card").each((_, el) => {
      const $el = $(el);
      ...
      if (title && title.length > 2) {
        articles.push({
          title,
          imageUrls: img ? [img] : [],
          sourceUrl: link.startsWith("http") ? link : `https://i-d.co${link}`,
        });
      }
    });
