...
export class EsquireChinaScraper extends BaseScraper {
  name = "Esquire China";
  scrapeUrl = "https://www.esquire.com.cn/";
    ...
    $(".article-item, .post-item, .news-item").each((_, el) => {
      ...
      if (title && title.length > 2) {
        articles.push({
          title,
          imageUrls: img ? [img] : [],
          sourceUrl: link.startsWith("http") ? link : `https://www.esquire.com.cn${link}`,
        });
      }
    });
